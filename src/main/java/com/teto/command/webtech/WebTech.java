package com.teto.command.webtech;
...
public class WebTech extends AbstractCommand<Void> implements ITargetNode, IScripts, IMerge, IAttackVector {
    private final TargetNode node;
    private final Target target;

    public WebTech(TargetNode node, Target target) {
        this.node = node;
        this.target = target;
    }
    ...
    public Optional<Void> apply(Context ctx) {
        ...
        Optional<Script> script = getScript(ctx, Provenance.WebTech);
        ...
        if(fileName != null) {
            WebTechParser parser = new WebTechParser();
